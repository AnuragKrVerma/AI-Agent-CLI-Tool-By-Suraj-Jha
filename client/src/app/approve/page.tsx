...
const DeviceApprovalPage = () => {
  ...
  if (isPending) {
    return (
      <div className="flex flex-col items-center justify-content h-screen bg-background">
        <Spinner />
      </div>
    );
  }
  ...
  const handleAprrove = async () => {
    ...
    try {
      toast.loading("Approving device...", { id: "approving" });
  ...
  const handleDeny = async () => {
    ...
    try {
      toast.loading("Denying device...", { id: "denying" });
      await authClient.device.deny({
        userCode: userCode!,
      });
      toast.dismiss("denying");
  ...
  return (
    <div className="flex min-h-screen item-center justify-center bg-background font-sans">
          ...
          <div className="border-2 border-dashed border-zinc-700 rounded-2xl p-6 bg-zinc-900/50 backdrop-blur-sm space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-zinc-600 text-center ">
                Share the code with the requesting device
              </p>
            </div>
          </div>
          {/* Security Info Card */}
