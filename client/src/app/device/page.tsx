...
const DeviceAuthorizationPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        `Invalid or expired code ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-2 ">
                  Find this code on this device you want to authorize
                </p>
              </div>
              {/* Error Message */}
              {error && (
                <div className="p-3 rounded-lg bg-red-950 border border-red-900  text-rose-200 text-sm">
                  {error}
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-3 bg-zinc-100 text-zinc-950 font-semibold rounded-lg hover:bg-zinc-200 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <div className="p-4 bg-zinc-900 border-2 border-dashed border-zinc-700 rounded-lg">
                <p className="text-sm text-muted-foreground leading-relaxed">
                  This code is uniqued to your device & will expire shortly.
                  Keep it confidential & never share with anyone
                </p>
              </div>
            </div>
          </form>
